...
export default function RecordTab({ trip }) {
  ...
  if (loading) return <div className="records-loading">기록을 불러오는 중…</div>
...
function RecordEditorModal({ trip, data, record, defaultDayId, onClose, onSaved }) {
  ...
  return (
    <div className="modal-backdrop">
      <div className="create-sheet record-editor-sheet">
        <div className="sheet-handle" />
        <div className="sheet-head"><button onClick={onClose}>취소</button><span>{record ? '기록 수정' : '기록 추가'}</span><div /></div>
        <div className="sheet-body record-form">
          ...
          <div className="record-file-box">
            <div className="record-file-head"><div><strong>사진 · 파일</strong><small>사진은 여러 장, 티켓·영수증은 이미지나 PDF로 첨부할 수 있어요.</small></div><label className="record-file-add"><Plus size={16} /> 추가<input type="file" accept="image/*,application/pdf" multiple onChange={(e) => setFiles(Array.from(e.target.files || []))} /></label></div>
...
function RecordDetailModal({ record, onClose, onEdit, onDeleted }) {
  ...
  return (
    <div className="modal-backdrop">
      <div className="create-sheet record-detail-sheet">
        <div className="sheet-handle" />
