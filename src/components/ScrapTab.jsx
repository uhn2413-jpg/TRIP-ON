...
export default function ScrapTab({ trips }) {
  ...
  useEffect(() => {
    ...
    let timer = null
    const channel = supabase.channel(`trip-on-scraps:${Math.random().toString(36).slice(2)}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'trip_on_scraps' }, () => {
        window.clearTimeout(timer)
        timer = window.setTimeout(() => reload(), 180)
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'trip_on_places' }, () => {
        window.clearTimeout(timer)
        timer = window.setTimeout(() => reload(), 180)
      })
      .subscribe()
  ...
  const editableTrips = useMemo(() => trips.filter((trip) => trip.accessRole !== 'viewer'), [trips])
  const scrapIsReadOnly = (scrap) => Boolean(scrap?.trip_id && trips.find((trip) => trip.id === scrap.trip_id)?.accessRole === 'viewer')
    ...
    <>
      ...
      {viewer && (
        <ScrapDetail
          ...
          readOnly={scrapIsReadOnly(viewer)}
        />
      )}
...
function ScrapDetail({ scrap, trips, onClose, onEdit, onSchedule, onDeleted, readOnly = false }) {
  const format = getScrapFormat(scrap)
  const tags = getScrapTags(scrap)
  const Icon = formatIcons[format] || FileText
    ...
    <SheetBackdrop onRequestClose={onClose}>
      <div className="create-sheet scrap-detail-sheet" onPointerDown={(e) => e.stopPropagation()}>
        ...
        <div className="sheet-body scrap-detail-body">
          ...
          {scrap.trip_on_places?.latitude != null && scrap.trip_on_places?.longitude != null && (() => { const mappedPlace = placeFromRow(scrap.trip_on_places); return <button className="outline-wide" onClick={() => window.open(externalMapUrl(mappedPlace), '_blank', 'noopener,noreferrer')}><MapPin size={17} /> {externalMapLabel(mappedPlace)}</button> })()}
          ...
          {!readOnly && <div className="scrap-detail-actions">
            <button onClick={onEdit}><Pencil size={17} /> 수정</button>
            <button className="danger" onClick={remove}><Trash2 size={17} /> 삭제</button>
          </div>}
          {canSchedule && !readOnly && <button className="sheet-primary" onClick={onSchedule}><CalendarPlus size={18} /> 일정에 추가</button>}
        </div>
      </div>
    </SheetBackdrop>
  )
