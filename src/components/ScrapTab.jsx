...
export default function ScrapTab({ trips }) {
  ...
  const filtered = useMemo(() => scraps.filter((scrap) => {
    const typeOk = filter === 'all'
      || scrap.type === filter
      || (filter === 'link-memo' && ['link', 'reference', 'memo', 'other'].includes(scrap.type))
...
function ScrapEditor({ scrap, trips, onClose, onSaved }) {
  ...
  const initialSnapshot = JSON.stringify({ type: scrap?.type || 'place', title: scrap?.title || '', tripId: scrap?.trip_id || '', country: place?.country || '', city: place?.city || '', address: place?.address || '', timezone: place?.timezone || 'Asia/Seoul', url: scrap?.url || '', memo: scrap?.memo || '', mapKey: place?.provider_place_id || place?.id || '' })
  const currentSnapshot = JSON.stringify({ type, title, tripId, country, city, address, timezone, url, memo, mapKey: mapPlace?.providerPlaceId || mapPlace?.dbId || '' })
  ...
  const save = async () => {
    ...
    try {
      await saveScrap({
        ...
        type, title, tripId, country, city, address, timezone, url, memo, mapPlace,
      })
      await onSaved()
    } catch (e) {
      setError(e.message)
