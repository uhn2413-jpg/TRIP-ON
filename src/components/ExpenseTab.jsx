...
export default function ExpenseTab({ trip, refreshKey = 0, readOnly = false }) {
  ...
  return (
    <>
      ...
      {!readOnly && <div className="expense-top-actions">
      ...
      <section className="settlement-card">
        ...
        {settlement.unsettledExpenseCount === 0 ? (
          ...
          <>
            ...
            {!readOnly && <button className="settlement-complete" disabled={settling} onClick={() => markSettlement(true)}><Check size={16} /> {settling ? '처리 중…' : '정산 완료 처리'}</button>}
          </>
        )}
        {!readOnly && data.expenses.some((expense) => expense.is_shared && (expense.shares || []).some((share) => share.is_settled)) && (
          <button className="settlement-reopen" disabled={settling} onClick={() => markSettlement(false)}>정산 다시 열기</button>
        )}
      </section>

      {!readOnly && budgetOpen && <BudgetModal initial={budget} onClose={() => setBudgetOpen(false)} onSave={async (amount) => { await saveBudget(trip.id, amount); setBudgetOpen(false); await reload() }} />}
      {!readOnly && editor && <ExpenseEditorModal trip={trip} data={data} expense={editor.__new ? null : editor} onClose={() => setEditor(null)} onSaved={async () => { setEditor(null); await reload() }} />}
      {viewer && <ExpenseDetailModal expense={viewer} onClose={() => setViewer(null)} onEdit={() => { setEditor(viewer); setViewer(null) }} onDeleted={async () => { setViewer(null); await reload() }} readOnly={readOnly} />}
    </>
  )
}
...
function ExpenseDetailModal({ expense, onClose, onEdit, onDeleted, readOnly = false }) {
  ...
  return (
    <SheetBackdrop onRequestClose={onClose}>
      <div className="create-sheet expense-detail-sheet" onPointerDown={(e) => e.stopPropagation()}>
        <div className="sheet-handle" /><div className="sheet-head"><button onClick={onClose}>닫기</button><span>지출 상세</span>{readOnly ? <div /> : <button onClick={onEdit}><Pencil size={17} /></button>}</div>
