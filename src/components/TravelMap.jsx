...
export function MapCanvas({ points = [], connectPoints = false, pathSegments = [], className = '' }) {
  ...
  const [override, setOverride] = useState(() => getDefaultMapProvider())
  ...
  const autoProvider = canUseKakao
    ? 'kakao'
    : canUseGoogle
      ? 'google'
      : ''
  const provider = override === 'auto'
    ? autoProvider
    : override === 'kakao'
      ? (canUseKakao ? 'kakao' : autoProvider)
      : override === 'google'
        ? (canUseGoogle ? 'google' : autoProvider)
        : autoProvider
