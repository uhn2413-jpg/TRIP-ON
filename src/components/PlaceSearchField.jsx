...
export default function PlaceSearchField({
  ...
  const [providerMode, setProviderMode] = useState(() => ['kakao', 'google'].includes(value?.provider) ? value.provider : 'auto')
    ...
    <div className="place-search-box hybrid-place-search">
      ...
      {selected ? (
        <div className="selected-place-card">
          ...
          <button type="button" onClick={() => { onChange(null); setQuery(''); setResults([]); setProviderMode('auto') }} aria-label="장소 연결 해제"><X size={17} /></button>
        </div>
      ) : (
        <>
