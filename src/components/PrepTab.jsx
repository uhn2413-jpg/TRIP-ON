...
export default function PrepTab({ trip }) {
  ...
  const openFile = async (file) => {
    try {
      const url = await openReservationFile(file.storage_path)
  ...
  const reservationSection = (
    <section className="prep-section">
      <div className="prep-section-head">
        ...
        <button className="prep-add-btn" onClick={() => setReservationModal({})}><Plus size={16} /> 추가</button>
      </div>
      {data.reservations.length === 0 ? (
        ...
        <div className="reservation-list">
          {data.reservations.map((reservation) => {
            const link = reservation.trip_on_reservation_itinerary_links?.[0]?.itinerary_item_id
            const itinerary = data.itinerary.find((item) => item.id === link)
            const fileCount = reservation.trip_on_reservation_files?.length || 0
            return (
              <article className="reservation-card" key={reservation.id}>
                <button className="reservation-main" onClick={() => setReservationModal(reservation)}>
                  <div className="reservation-type-icon"><Plane size={18} /></div>
                  <div className="reservation-copy">
                    ...
                    {(itinerary || fileCount > 0) && <small className="reservation-linkline">{itinerary ? `일정 연결 · ${itinerary.day ? `DAY ${itinerary.day.day_number} ` : ''}${itinerary.title}` : ''}{itinerary && fileCount ? '  ·  ' : ''}{fileCount ? `문서 ${fileCount}개` : ''}</small>}
                  </div>
                  <ChevronRight size={18} />
                </button>
                <button className="reservation-delete" disabled={working} onClick={() => removeReservation(reservation)} aria-label="예약 삭제"><Trash2 size={15} /></button>
              </article>
            )
          })}
        </div>
  ...
  const checklistSection = (
    <>
      <PrepChecklist title="할 일" kicker="TO DO" items={todos} onToggle={toggle} onAdd={() => setPrepModal({ type: 'todo' })} onEdit={setPrepModal} onDelete={removePrep} empty="환전·보험·예약 같은 할 일을 추가해보세요." />
      <PrepChecklist title="준비물" kicker="PACKING" items={packing} onToggle={toggle} onAdd={() => setPrepModal({ type: 'packing' })} onEdit={setPrepModal} onDelete={removePrep} empty="여권·충전기·상비약 같은 준비물을 적어둘 수 있어요." />
    </>
  )
  ...
  return (
    <div className="prep-tab">
        ...
        <>
              ...
              <div className="voucher-grid">
                {allFiles.map((file) => (
                  <button key={file.id} onClick={() => openFile(file)}>
                    <span className="voucher-icon">{file.mime_type === 'application/pdf' ? <FileText size={18} /> : <Paperclip size={18} />}</span>
                    <strong>{file.file_name}</strong>
                    <small>{file.reservationTitle}</small>
                  </button>
                ))}
              </div>
            )}
          ...
          <section className="prep-section">
            <div className="prep-section-head">
              ...
              <button className="prep-add-btn" onClick={() => setInfoModal({})}><Plus size={16} /> 추가</button>
            </div>
              ...
              <div className="trip-info-list">
                {data.infoItems.map((item) => (
                  <div className="trip-info-row" key={item.id}>
                    <button onClick={() => setInfoModal(item)}>
                      <span>{item.category || '정보'}</span><strong>{item.label}</strong><small>{item.value || item.memo || '내용 없음'}</small>
                    </button>
                    <button onClick={() => removeInfo(item)} aria-label="여행 정보 삭제"><Trash2 size={15} /></button>
                  </div>
                ))}
              </div>
            )}
      ...
      {prepModal && <PrepItemModal initial={prepModal} tripId={trip.id} onClose={() => setPrepModal(null)} onSaved={async () => { setPrepModal(null); await reload() }} />}
      {reservationModal && <ReservationModal initial={reservationModal} trip={trip} itinerary={data.itinerary} onClose={() => setReservationModal(null)} onSaved={async () => { setReservationModal(null); await reload() }} onMessage={setMessage} />}
      {infoModal && <TripInfoModal initial={infoModal} tripId={trip.id} onClose={() => setInfoModal(null)} onSaved={async () => { setInfoModal(null); await reload() }} />}
    </div>
  )
}

function PrepChecklist({ title, kicker, items, onToggle, onAdd, onEdit, onDelete, empty }) {
  const done = items.filter((item) => item.is_done).length
  return (
    <section className="prep-section">
      <div className="prep-section-head">
        <div><span className="prep-kicker">{kicker}</span><h3>{title}</h3></div>
        <div className="prep-head-actions"><span className="prep-count">{done}/{items.length}</span><button className="prep-add-btn" onClick={onAdd}><Plus size={16} /> 추가</button></div>
      </div>
        ...
        <div className="checklist-list">
          {items.map((item) => (
            <div className={`checklist-row ${item.is_done ? 'done' : ''}`} key={item.id}>
              <button className="checklist-check" onClick={() => onToggle(item)} aria-label={item.is_done ? '완료 해제' : '완료'}>{item.is_done && <Check size={15} strokeWidth={3} />}</button>
              <button className="checklist-title" onClick={() => onEdit(item)}>{item.title}</button>
              <button className="checklist-delete" onClick={() => onDelete(item)} aria-label="삭제"><X size={15} /></button>
            </div>
          ))}
        </div>
      )}
...
function ReservationModal({ initial, trip, itinerary, onClose, onSaved, onMessage }) {
  ...
  const [startLocal, setStartLocal] = useState(initial.start_at ? isoToLocalInput(initial.start_at, initial.start_timezone || DEFAULT_TIMEZONE) : '')
  const [endLocal, setEndLocal] = useState(initial.end_at ? isoToLocalInput(initial.end_at, initial.end_timezone || initial.start_timezone || DEFAULT_TIMEZONE) : '')
  ...
  const [existingFiles, setExistingFiles] = useState(initial.trip_on_reservation_files || [])
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  const initialSnapshot = JSON.stringify({ type: initial.type || 'flight', title: initial.title || '', provider: initial.provider || '', reservationNumber: initial.reservation_number || '', startTimezone: initial.start_timezone || DEFAULT_TIMEZONE, endTimezone: initial.end_timezone || initial.start_timezone || DEFAULT_TIMEZONE, startLocal: initial.start_at ? isoToLocalInput(initial.start_at, initial.start_timezone || DEFAULT_TIMEZONE) : '', endLocal: initial.end_at ? isoToLocalInput(initial.end_at, initial.end_timezone || initial.start_timezone || DEFAULT_TIMEZONE) : '', amount: String(initial.amount ?? ''), currency: initial.currency || 'KRW', url: initial.url || '', memo: initial.memo || '', status: initial.status || 'confirmed', itineraryItemId: initial.trip_on_reservation_itinerary_links?.[0]?.itinerary_item_id || '' })
  const currentSnapshot = JSON.stringify({ type, title, provider, reservationNumber, startTimezone, endTimezone, startLocal, endLocal, amount: String(amount), currency, url, memo, status, itineraryItemId })
  ...
  const save = async () => {
    ...
    try {
      const saved = await saveReservation({
        tripId: trip.id,
        draft: { id: initial.id, type, title, provider, reservationNumber, startLocal, endLocal, startTimezone, endTimezone, amount, currency, url, memo, status, itineraryItemId },
      })
      if (files.length) await uploadReservationFiles({ tripId: trip.id, reservationId: saved.id, files })
      await onSaved()
    } catch (e) {
      setError(e.message)
    } finally { setSaving(false) }
  ...
  const openFile = async (file) => {
    try { window.open(await openReservationFile(file.storage_path), '_blank', 'noopener,noreferrer') } catch (e) { onMessage(e.message) }
  }
  const removeFile = async (file) => {
    if (!window.confirm(`${file.file_name} 문서를 삭제할까요?`)) return
    try {
    ...
    <SheetBackdrop onRequestClose={requestClose}>
      <div className="create-sheet reservation-sheet" onPointerDown={(e) => e.stopPropagation()}>
        ...
        <div className="sheet-body tidy-form">
          ...
          <label>예약 이름<input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="예: 파리 호텔 / 유로스타" /></label>
          <div className="form-two">
            <label>예약처 <span className="optional">선택</span><input value={provider} onChange={(e) => setProvider(e.target.value)} placeholder="예: Booking.com" /></label>
          ...
          <div className="form-two">
            <label>시작 시간대<select value={startTimezone} onChange={(e) => { setStartTimezone(e.target.value); if (!initial.end_timezone) setEndTimezone(e.target.value) }}>{timezoneOptions.map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
          ...
          <label>일정 연결 <span className="optional">선택</span>
            <select value={itineraryItemId} onChange={(e) => setItineraryItemId(e.target.value)}>
              ...
              {itinerary.map((item) => <option value={item.id} key={item.id}>{item.day ? `DAY ${item.day.day_number} · ` : ''}{item.title}</option>)}
            </select>
          </label>
          ...
          <div className="file-field">
            ...
            {existingFiles.length > 0 && <div className="existing-files">{existingFiles.map((file) => <div key={file.id}><button onClick={() => openFile(file)}><Paperclip size={15} />{file.file_name}</button><button onClick={() => removeFile(file)} aria-label="첨부 삭제"><X size={14} /></button></div>)}</div>}
            <label className="file-upload-button"><Paperclip size={17} /> 파일 선택<input type="file" accept="image/*,application/pdf" multiple onChange={(e) => setFiles(Array.from(e.target.files || []))} /></label>
            {files.length > 0 && <small className="selected-files">새 파일 {files.length}개 선택됨 · {files.map((file) => file.name).join(', ')}</small>}
          </div>
