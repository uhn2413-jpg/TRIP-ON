...
function ReservationModal({ initial, trip, itinerary, onClose, onSaved, onMessage }) {
  ...
  const openFile = async (file) => {
    try { window.open(await openReservationFile(file), '_blank', 'noopener,noreferrer') } catch (e) { onMessage(e.message) }
  }
      ...
      <div className="create-sheet reservation-sheet" onPointerDown={(e) => e.stopPropagation()}>
        ...
        <div className="sheet-body tidy-form">
          ...
          <div className="file-field">
            ...
            <label className="file-upload-button"><Paperclip size={17} /> 파일 선택<input type="file" accept="image/*,.heic,.heif,application/pdf" multiple onChange={(e) => setFiles(Array.from(e.target.files || []))} /></label>
