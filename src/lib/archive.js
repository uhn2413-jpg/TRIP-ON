...
export async function readTimelineFile(file) {
  ...
  if (lower.endsWith('.zip')) {
    ...
    let entries
    try { entries = await unzipAsync(buffer) }
    catch { throw new Error('ZIP 압축을 풀지 못했어요. 파일이 손상되지 않았는지 확인해주세요.') }
  ...
  let parsed
  try { parsed = JSON.parse(text) } catch { throw new Error('JSON을 읽지 못했어요. Google 타임라인 내보내기 파일인지 확인해주세요.') }
  if (!Array.isArray(parsed?.semanticSegments)) {
    throw new Error('semanticSegments를 찾지 못했어요. 현재 Google 타임라인 내보내기 형식과 다른 파일 같아요.')
  }

  // 큰 Timeline 파일은 원본 객체를 계속 들고 있지 않고, 한 번만 정규화한 뒤 필요한 데이터만 유지한다.
  // 기간을 바꿔 미리 볼 때도 같은 세그먼트를 다시 파싱하지 않아 모바일 메모리와 CPU 사용량을 줄인다.
  const normalized = parsed.semanticSegments.map(normalizeSegment).filter(Boolean)
  const meta = inspectTimeline(normalized)
  return { data: normalized, meta, sourceBytes: file.size || 0 }
}
...
function normalizedTimelineData(data) {
  if (Array.isArray(data)) return data
  return (data?.semanticSegments || []).map(normalizeSegment).filter(Boolean)
}

export function inspectTimeline(data) {
  const normalized = normalizedTimelineData(data)
...
export function previewTimeline(data, startDate, endDate) {
  ...
  const segments = normalizedTimelineData(data)
