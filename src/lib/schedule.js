...
export async function fetchSchedule(trip) {
  ...
  const { data: items, error } = await supabase
    ...
    .select(`
      ...
      import_source, import_source_ref, linked_plan_id, group_parent_id, is_visit_group, group_title, created_at, updated_at,
  ...
  for (const item of rows) {
    if (item.linked_plan_id && byId.has(item.linked_plan_id)) {
      ...
      item.linked_plan = { id: plan.id, title: plan.title, group_title: plan.group_title, category: plan.category, city: plan.city, start_at: plan.start_at, start_timezone: plan.start_timezone, is_visit_group: plan.is_visit_group }
...
export async function linkActualToPlan(actualId, plannedId) {
  const { error } = await supabase
    .from('trip_on_itinerary_items')
...
export async function replacePlanActualLinks(plannedId, actualIds = []) {
