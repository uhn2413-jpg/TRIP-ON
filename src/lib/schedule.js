...
export async function fetchSchedule(trip) {
  ...
  const [itemsResult, accommodationsResult] = await Promise.all([
    supabase
      .from('trip_on_itinerary_items')
      .select(`
        ...
        import_source, import_source_ref, linked_plan_id, link_display_mode, group_parent_id, is_visit_group, group_title, created_at, updated_at,
        trip_on_places:trip_on_places!trip_on_itinerary_items_place_id_fkey ( id, name, city, country, address, latitude, longitude, timezone, provider, provider_place_id, map_url ),
    ...
    supabase
      ...
      .select('id, title, provider, start_at, end_at, start_timezone, end_timezone, is_start_time_unspecified, is_end_time_unspecified, status')
      .eq('trip_id', trip.id)
  ...
  for (const item of rows) {
    if (item.linked_plan_id && byId.has(item.linked_plan_id)) {
      const plan = byId.get(item.linked_plan_id)
      item.linked_plan = { id: plan.id, title: plan.title, group_title: plan.group_title, category: plan.category, city: plan.city, start_at: plan.start_at, start_timezone: plan.start_timezone, is_visit_group: plan.is_visit_group, link_display_mode: plan.link_display_mode || 'plan' }
      plan.linked_actuals.push(item)
    }
    if (item.group_parent_id && byId.has(item.group_parent_id)) {
      const parent = byId.get(item.group_parent_id)
      item.group_parent = { id: parent.id, title: parent.title, group_title: parent.group_title, city: parent.city }
      parent.group_children.push(item)
    }
  }

  const actualSort = (a, b) => {
    const aTime = a.actual_start_at || a.start_at || ''
    const bTime = b.actual_start_at || b.start_at || ''
    return String(aTime).localeCompare(String(bTime)) || (a.sort_order ?? 0) - (b.sort_order ?? 0)
  }
  for (const item of rows) {
    item.linked_actuals.sort(actualSort)
    item.group_children.sort(actualSort)
  }

  return { days, items: rows, accommodations: accommodationsResult.data || [] }
}
...
export function dateInZone(iso, timeZone = DEFAULT_TIMEZONE) {
  if (!iso) return ''
  try {
    const parts = Object.fromEntries(
...
export async function saveItineraryItem({ trip, day, draft }) {
  ...
  const payload = {
    ...
    actual_start_at: draft.actualStartTime ? zonedLocalToIso(draft.actualStartDate || dayDate, draft.actualStartTime, timezone) : null,
    actual_end_at: draft.actualEndTime ? zonedLocalToIso(draft.actualEndDate || dayDate, draft.actualEndTime, endTimezone) : null,
...
export function isTimelineActualItem(item) {
  return item?.import_source === 'google_timeline'
}

export async function updatePlanLinkDisplayMode(plannedId, displayMode = 'plan') {
  if (!plannedId) return
  const { error } = await supabase
    .from('trip_on_itinerary_items')
    .update({ link_display_mode: displayMode || 'plan', updated_at: new Date().toISOString() })
    .eq('id', plannedId)
  if (error) throw error
}

export async function linkActualToPlan(actualId, plannedId, displayMode = null) {
  const { error } = await supabase
    .from('trip_on_itinerary_items')
    .update({ linked_plan_id: plannedId || null, updated_at: new Date().toISOString() })
    .eq('id', actualId)
  if (error) throw error
  if (plannedId && displayMode) await updatePlanLinkDisplayMode(plannedId, displayMode)
}

export async function replacePlanActualLinks(plannedId, actualIds = [], displayMode = 'plan') {
  const cleanIds = [...new Set((actualIds || []).filter(Boolean))]

  const { error: clearError } = await supabase
    .from('trip_on_itinerary_items')
    .update({ linked_plan_id: null, updated_at: new Date().toISOString() })
    .eq('linked_plan_id', plannedId)
  if (clearError) throw clearError

  await updatePlanLinkDisplayMode(plannedId, displayMode)

  if (!cleanIds.length) return
  const { error: linkError } = await supabase
    .from('trip_on_itinerary_items')
    .update({ linked_plan_id: plannedId, updated_at: new Date().toISOString() })
    .in('id', cleanIds)
  if (linkError) throw linkError
}

export async function saveVisitGroup({ parentId, childIds = [], title = '' }) {
  if (!parentId) throw new Error('대표 장소를 선택해주세요.')
  const cleanChildren = [...new Set((childIds || []).filter((id) => id && id !== parentId))]
  if (!cleanChildren.length) throw new Error('대표 장소 외에 한 곳 이상을 묶어주세요.')

  const { error: clearError } = await supabase
    .from('trip_on_itinerary_items')
    .update({ group_parent_id: null, updated_at: new Date().toISOString() })
  ...
  const { error: childError } = await supabase
    .from('trip_on_itinerary_items')
    .update({ group_parent_id: parentId, updated_at: new Date().toISOString() })
    .in('id', cleanChildren)
  if (childError) throw childError
}

export async function dissolveVisitGroup(parentId) {
  const { error: childError } = await supabase
    .from('trip_on_itinerary_items')
    .update({ group_parent_id: null, updated_at: new Date().toISOString() })
    .eq('group_parent_id', parentId)
  ...
  const { error: parentError } = await supabase
    .from('trip_on_itinerary_items')
    .update({ is_visit_group: false, group_title: null, updated_at: new Date().toISOString() })
    .eq('id', parentId)
  if (parentError) throw parentError
}

export async function removeItemFromVisitGroup(itemId) {
  const { error } = await supabase
    .from('trip_on_itinerary_items')
    .update({ group_parent_id: null, updated_at: new Date().toISOString() })
    .eq('id', itemId)
