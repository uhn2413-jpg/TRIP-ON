...
export async function savePlaceRecord(place) {
  ...
  if (payload.provider_place_id) {
    ...
    const ownExisting = (matches || []).find((row) => row.owner_id === userId)
    if (ownExisting) {
      const { data, error } = await supabase.from('trip_on_places').update(payload).eq('id', ownExisting.id).select('*').single()
      if (error) throw error
      return data
    }
  }
...
export async function fetchTripMapPoints(tripId) {
  const [itemsRes, scrapsRes] = await Promise.all([
    supabase.from('trip_on_itinerary_items').select(`
      ...
      trip_on_places:trip_on_places!trip_on_itinerary_items_place_id_fkey ( ${placeFields} ),
