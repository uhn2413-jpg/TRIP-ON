...
export async function fetchPrepData(tripId) {
  const [prepRes, reservationsRes, infoRes, itineraryRes, daysRes] = await Promise.all([
    ...
    supabase
      ...
      .select(`
        ...
        is_start_time_unspecified, is_end_time_unspecified,
        amount, currency, url, memo, status, created_at, updated_at,
        trip_on_reservation_itinerary_links ( itinerary_item_id ),
        trip_on_reservation_files ( id, storage_path, file_name, display_name, mime_type, created_at )
      `)
      .eq('trip_id', tripId)
    ...
    supabase
      ...
      .select('id, title, trip_day_id, start_at, actual_start_at, start_timezone, actual_start_timezone, is_time_unscheduled, sort_order, import_source, linked_plan_id, group_parent_id')
      .eq('trip_id', tripId)
      .order('sort_order', { ascending: true }),
    supabase
  ...
  const itinerary = (itineraryRes.data || [])
    .map((item) => ({ ...item, day: dayMap.get(item.trip_day_id) || null }))
    .filter((item) => !item.group_parent_id && !item.linked_plan_id)
    .sort((a, b) => {
...
export async function saveReservation({ tripId, draft }) {
  const payload = {
    ...
    start_at: draft.startDate
      ? zonedLocalToIso(draft.startDate, draft.startTime || '00:00', draft.startTimezone || DEFAULT_TIMEZONE)
      : dateTimeToIso(draft.startLocal, draft.startTimezone),
    end_at: draft.endDate
      ? zonedLocalToIso(draft.endDate, draft.endTime || '00:00', draft.endTimezone || draft.startTimezone || DEFAULT_TIMEZONE)
      : dateTimeToIso(draft.endLocal, draft.endTimezone || draft.startTimezone),
    start_timezone: draft.startTimezone || DEFAULT_TIMEZONE,
    end_timezone: draft.endTimezone || draft.startTimezone || DEFAULT_TIMEZONE,
    is_start_time_unspecified: Boolean(draft.isStartTimeUnspecified),
    is_end_time_unspecified: Boolean(draft.isEndTimeUnspecified),
  ...
  const itineraryItemIds = [...new Set((draft.itineraryItemIds || [draft.itineraryItemId]).filter(Boolean))]
  if (itineraryItemIds.length) {
    const { error: linkError } = await supabase.from('trip_on_reservation_itinerary_links').insert(
      itineraryItemIds.map((itineraryItemId) => ({
...
export async function uploadReservationFiles({ tripId, reservationId, files }) {
  ...
  for (const entry of files) {
    ...
    const displayName = entry && typeof entry === 'object' && 'displayName' in entry ? entry.displayName : ''
...
export async function openReservationFile(fileOrPath) {
  const storagePath = typeof fileOrPath === 'string' ? fileOrPath : fileOrPath?.storage_path
  if (!storagePath) throw new Error('파일 경로를 찾지 못했어요.')
  const { data, error } = await supabase.storage.from(BUCKET).createSignedUrl(storagePath, 300)
  if (error) throw error
  if (typeof fileOrPath === 'string') return data.signedUrl
  return await previewableImageUrl({
    url: data.signedUrl,
    fileName: fileOrPath.file_name,
    mimeType: fileOrPath.mime_type,
    cacheKey: storagePath,
  }) || data.signedUrl
}

export async function updateReservationFileMeta(fileId, { displayName } = {}) {
