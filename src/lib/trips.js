...
async function normalizeCoverUpload(file) {
  ...
  try {
    ...
    if (!(blob instanceof Blob)) throw new Error('HEIC 변환 결과가 올바르지 않아요.')
...
async function resolveCoverImage(pathOrUrl) {
  ...
  const { data, error } = await supabase.storage.from(FILE_BUCKET).createSignedUrl(pathOrUrl, COVER_SIGN_SECONDS)
