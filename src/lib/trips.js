...
export async function uploadTripCover(tripId, file) {
  ...
  if (!file.type?.startsWith('image/')) throw new Error('커버에는 이미지 파일만 사용할 수 있어요.')
  if (file.size > 12 * 1024 * 1024) throw new Error('커버 사진은 12MB 이하만 올릴 수 있어요.')
  ...
  const path = `${userId}/${tripId}/cover/${crypto.randomUUID?.() || Date.now()}-${safeFileName(file.name)}`
