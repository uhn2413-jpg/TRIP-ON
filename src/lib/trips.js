...
async function convertHeicBlob(blob, quality = 0.9) {
...
async function normalizeCoverUpload(file) {
  if (!isHeicFile(file)) return file

  try {
    const blob = await convertHeicBlob(file, 0.9)
    const nextName = String(file.name || 'cover.heic').replace(/\.(heic|heif)$/i, '') + '.jpg'
    return new File([blob], nextName, { type: 'image/jpeg', lastModified: Date.now() })
  } catch (error) {
    console.error('HEIC cover conversion failed', error)
...
async function resolveCoverImage(pathOrUrl) {
  ...
  const heicLike = /\.(heic|heif)(?:$|[?#])/i.test(String(pathOrUrl))
  if (!heicLike) {
    coverPreviewCache.set(pathOrUrl, directUrl)
    return directUrl
  }

  // 이전 버전에서 이미 HEIC 원본이 저장된 여행도 다시 업로드할 필요 없이
  // 브라우저에서 미리보기용 JPEG로 변환해 보여준다.
  try {
    const response = await fetch(directUrl)
    if (!response.ok) throw new Error('커버 원본을 불러오지 못했어요.')
    const source = await response.blob()
    const converted = await convertHeicBlob(source, 0.88)
    const objectUrl = URL.createObjectURL(converted)
    coverPreviewCache.set(pathOrUrl, objectUrl)
    return objectUrl
  } catch (error) {
    console.warn('Stored HEIC cover preview conversion failed', error)
    return null
  }
}
...
export async function uploadTripCover(tripId, file) {
  ...
  if (!looksLikeImage) throw new Error('커버에는 이미지 파일만 사용할 수 있어요.')
  if (file.size > 20 * 1024 * 1024) throw new Error('원본 사진은 20MB 이하만 선택할 수 있어요.')

  const uploadFile = await normalizeCoverUpload(file)
  if (uploadFile.size > 12 * 1024 * 1024) throw new Error('변환된 커버 사진은 12MB 이하만 올릴 수 있어요.')
