...
import { createTripRecord, deleteTripRecord, fetchTrips, updateTripRecord } from './lib/trips'
      ...
      <>
        ...
        {editingTrip && (
          <TripEditorModal
            ...
            onSave={saveTrip}
            onDelete={() => deleteTrip(editingTrip)}
          />
        )}
        {colorEditingTrip && (
          <ColorEditorModal
...
function TripDetail({ trip, onBack, onEdit, onColorEdit, onTripUpdated }) {
...
function ItineraryEditorModal({ trip, day, item, defaultUnscheduled = false, onClose, onSaved }) {
  ...
  const initialSnapshot = JSON.stringify({
    ...
    transportType: item?.transport_type || '', travelMinutes: item?.travel_minutes || '', expectedCost: item?.expected_cost || '',
    expectedCurrency: item?.expected_currency || 'KRW', memo: item?.memo || '', rating: item?.rating ? String(item.rating) : '', mapKey: item?.trip_on_places?.provider_place_id || item?.trip_on_places?.id || '',
  })
  const currentSnapshot = JSON.stringify({ title, category, city, isTimeUnscheduled, timezone, endTimezone, startTime, endTime, actualStartTime, actualEndTime, transportType, travelMinutes, expectedCost, expectedCurrency, memo, rating, mapKey: mapPlace?.providerPlaceId || mapPlace?.dbId || '' })
  ...
  const save = async () => {
    ...
    try {
      await saveItineraryItem({
        ...
        draft: {
          id: item?.id,
          title, category, city, place: mapPlace, isTimeUnscheduled,
...
function ColorEditorModal({ trip, onClose, onSave }) {
  const initialColor = normalizeTrip(trip).coverColor
  ...
  return (
    <SheetBackdrop onRequestClose={requestClose}>
      <div className="create-sheet color-sheet" onPointerDown={(e) => e.stopPropagation()}>
        <div className="sheet-handle" />
        <div className="sheet-head"><button onClick={requestClose}>취소</button><span>여행 색상</span><div /></div>
        <div className="sheet-body">
...
function TripEditorModal({ initialTrip, onClose, onSave, onDelete }) {
      ...
      <div className="create-sheet" onPointerDown={(e) => e.stopPropagation()}>
        ...
        {step === 3 && (
          <div className="sheet-body">
            ...
            {editing && onDelete && <button type="button" className="trip-delete-button" onClick={onDelete}><Trash2 size={18} /> 이 여행 삭제</button>}
          </div>
        )}
      </div>
    </SheetBackdrop>
