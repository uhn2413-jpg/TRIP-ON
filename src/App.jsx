...
function DataBackupScreen({ remoteMode, onBack, onBackupImported, onPreferencesImported }) {
  ...
  const download = async () => {
    ...
    try {
      ...
      setStatus(`JSON 백업을 만들었어요.${summaryText(backup) ? ` ${summaryText(backup)}` : ''}`)
    } catch (error) {
  ...
  const restore = async (event) => {
    ...
    setStatus('')
    try {
      const backup = await restoreTripOnBackup(file)
      ...
      setHistoryRows(rows)
      setStatus(`백업을 복원했어요.${summaryText(backup) ? ` ${summaryText(backup)}` : ''}`)
    } catch (error) {
  ...
  return (
    <div className="my-subpage data-backup-screen">
      ...
      <section className="section-block settings-card-block">
        ...
        <p className="preference-note">같은 계정에서 만든 JSON을 합쳐서 복원해요. 테마·지도·전환 설정도 함께 보관하지만, 이미지·PDF 원본 파일 자체는 JSON 안에 포함되지 않아요.</p>
