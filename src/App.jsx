...
import { createTripRecord, deleteTripRecord, fetchTrips, removeTripCover, updateTripRecord, uploadTripCover } from './lib/trips'
...
function App() {
  ...
  return (
    <div className="app-shell">
      <main className={`page root-tab-${tab}`}>
...
function TripDetail({ trip, onBack, onEdit, onDelete, onColorEdit, onTripUpdated }) {
      ...
      <main className="page detail-page">
        <div className="trip-hero" style={coverStyle(trip)}>
          ...
          <div className="hero-actions">
            ...
            <button className="hero-color" onClick={onColorEdit} aria-label="여행 커버 수정"><ImagePlus size={17} /><span>커버</span></button>
        ...
        <div className={`detail-content detail-tab-${active}`}>
          {active === 'overview' && <OverviewTab trip={trip} onTripUpdated={onTripUpdated} />}
          {active === 'schedule' && <ScheduleTab trip={trip} />}
          {active === 'prep' && <PrepTab trip={trip} />}
          {active === 'record' && <RecordTab trip={trip} />}
          {active === 'expense' && <ExpenseTab trip={trip} />}
        </div>
      </main>
...
function TripActionsSheet({ trip, onClose, onEdit, onDelete }) {
  return (
...
function OverviewTab({ trip, onTripUpdated }) {
  const state = getTripState(trip)
  ...
  if (state === 'past' || state === 'completed') return <ArchiveOverview trip={trip} onTripUpdated={onTripUpdated} />
  return (
    <>
...
function ScheduleTab({ trip }) {
  ...
  const rootItems = dayItems.filter((item) => !item.group_parent_id && !item.linked_plan_id)
...
function ScheduleCard({ item, onClick }) {
  ...
  const actualOnly = !hasPlannedTime && hasActualTime
  const startSource = actualOnly ? actualRange.start : item.start_at
  const endSource = actualOnly ? actualRange.end : item.end_at
  ...
  const summaryRecords = item.is_visit_group
    ? groupChildren
    : linkedActuals.flatMap((actual) => actual.is_visit_group ? [actual, ...(actual.group_children || [])] : [actual])
  const summaryNames = summaryRecords.map((entry) => entry.title).filter(Boolean)
  const title = scheduleDisplayTitle(item)

  return (
    <button className={`schedule-card status-${item.status} ${actualOnly ? 'actual-only' : ''} ${item.is_visit_group ? 'visit-group' : ''}`} onClick={onClick}>
      ...
      <div className="schedule-card-copy">
        <div className="schedule-card-top">
          ...
          <span className="schedule-card-badges">
            {item.is_visit_group && <span className="schedule-group-chip"><Layers3 size={11} /> {groupCount}곳</span>}
            {!item.is_visit_group && linkedCount > 0 && <span className="schedule-link-chip"><Link2 size={11} /> 실제 {linkedVisitCount}{linkedVisitCount > 1 ? '곳' : ''}</span>}
        ...
        {summaryNames.length > 0 && <div className="schedule-linked-summary">{summaryNames.slice(0, 3).join(' · ')}{summaryNames.length > 3 ? ` 외 ${summaryNames.length - 3}곳` : ''}</div>}
...
function ItineraryEditorModal({ trip, day, item, siblings = [], defaultUnscheduled = false, onClose, onSaved }) {
  ...
  useEffect(() => {
    ...
    if (originPlace?.name && destinationPlace?.name) setTitle(`${originPlace.name} → ${destinationPlace.name}`)
    else setTitle('')
  }, [category, originPlace?.name, destinationPlace?.name])
  ...
  const autoTransportTitle = originPlace?.name && destinationPlace?.name ? `${originPlace.name} → ${destinationPlace.name}` : ''
  const effectiveTitle = title.trim() || (category === 'transport' ? autoTransportTitle : '')
  const canSave = Boolean(effectiveTitle) && (isTimeUnscheduled || Boolean(startTime))
...
function ItineraryDetailModal({ item, day, onClose, onEdit, onLink, onEditGroup, onDissolveGroup, onOpenRelated, onChanged }) {
        ...
        <div className="sheet-body">
          ...
          {item.category !== 'transport' && item.trip_on_places && (
            <div className="itinerary-place-detail">
              <div><MapPin size={17} /><span><strong>{item.trip_on_places.name}</strong><small>{item.trip_on_places.address || [item.trip_on_places.country, item.trip_on_places.city].filter(Boolean).join(' · ')}</small></span></div>
              ...
              {showMiniMap && item.trip_on_places.latitude != null && item.trip_on_places.longitude != null && <div className="itinerary-mini-map"><MapCanvas className="mini-map-canvas" points={[{ id: item.trip_on_places.id, ...placeFromRow(item.trip_on_places) }]} /><button className="mini-map-external" onClick={() => window.open(googleMapsSearchUrl(placeFromRow(item.trip_on_places)), '_blank', 'noopener,noreferrer')}><MapPinned size={15} /> Google 지도에서 열기</button></div>}
            </div>
          )}
...
function ItineraryLinkModal({ item, day, dayItems, onClose, onSaved }) {
  ...
  const plans = dayItems
    .filter((candidate) => candidate.id !== item.id && !isTimelineActualItem(candidate) && !candidate.group_parent_id)
    .sort((a, b) => {
      const aTime = a.start_at || actualRangeForItem(a).start || ''
      const bTime = b.start_at || actualRangeForItem(b).start || ''
      return String(aTime).localeCompare(String(bTime)) || (a.sort_order ?? 0) - (b.sort_order ?? 0)
    })
  const actuals = dayItems
  ...
  const save = async () => {
    ...
    setError('')
    try {
      if (actualMode) await linkActualToPlan(item.id, selectedPlanId || null)
      else await replacePlanActualLinks(item.id, [...selectedActualIds])
      await onSaved()
    } catch (err) {
    ...
    <SheetBackdrop onRequestClose={onClose}>
      <div className="create-sheet schedule-link-sheet" onPointerDown={(e) => e.stopPropagation()}>
        ...
        <div className="sheet-body">
          ...
          <p className="relation-help">연결하면 일정표에서 계획과 실제가 한 카드에 표시돼요. 원본 일정과 Timeline 기록은 그대로 남아 있어요.</p>
          ...
          {actualMode ? (
            <div className="relation-choice-list single">
              ...
              {plans.map((plan) => {
                const time = plan.start_at ? timeInZone(plan.start_at, plan.start_timezone || DEFAULT_TIMEZONE) : '시간 미정'
                return <label key={plan.id} className={selectedPlanId === plan.id ? 'selected' : ''}><input type="radio" name="plan-link" checked={selectedPlanId === plan.id} onChange={() => setSelectedPlanId(plan.id)} /><time>{time}</time><span><strong>{scheduleDisplayTitle(plan)}</strong><small>{[categoryLabels[plan.category] || plan.category, plan.city].filter(Boolean).join(' · ') || '예정 일정'}</small></span></label>
              })}
            </div>
          ) : (
            <div className="relation-choice-list">
              {actuals.length === 0 ? <div className="relation-empty">연결할 실제 방문 기록이 아직 없어요.</div> : actuals.map((actual) => {
                ...
                const time = timeInZone(actual.actual_start_at, actual.actual_start_timezone || actual.start_timezone || DEFAULT_TIMEZONE)
                const linkedElsewhere = actual.linked_plan_id && actual.linked_plan_id !== item.id
                return <label key={actual.id} className={checked ? 'selected' : ''}><input type="checkbox" checked={checked} onChange={() => toggleActual(actual.id)} /><time>{time}</time><span><strong>{scheduleDisplayTitle(actual)}</strong><small>{linkedElsewhere ? '다른 예정 일정에 연결됨 · 선택하면 이 일정으로 옮겨와요.' : actual.city || 'Timeline 실제 방문'}</small></span></label>
              })}
            </div>
          )}
