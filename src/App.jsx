...
function ItineraryEditorModal({ trip, day, item, defaultUnscheduled = false, onClose, onSaved }) {
  ...
  const initialSnapshot = JSON.stringify({
    ...
    expectedCurrency: item?.expected_currency || 'KRW', memo: item?.memo || '', rating: item?.rating ? String(item.rating) : '',
  })
  const currentSnapshot = JSON.stringify({ title, category, city, isTimeUnscheduled, timezone, endTimezone, startTime, endTime, actualStartTime, actualEndTime, transportType, travelMinutes, expectedCost, expectedCurrency, memo, rating })
