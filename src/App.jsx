      ...
      <div className="create-sheet itinerary-sheet">
        ...
        <div className="sheet-body itinerary-form">
          ...
          <div className="rating-field">
            <div><strong>별점</strong><small>원하는 일정만 남기면 돼요 · 0.5 단위</small></div>
            <div className="rating-options">
              <button className={!rating ? 'active' : ''} onClick={() => setRating('')} type="button">없음</button>
              {Array.from({ length: 10 }, (_, i) => ((i + 1) / 2).toFixed(1)).map((value) => <button type="button" key={value} className={rating === value ? 'active' : ''} onClick={() => setRating(value)}>{value}</button>)}
            </div>
          </div>
