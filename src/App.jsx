...
import { downloadTripOnBackup, fetchTimelineImportHistory, restoreTripOnBackup } from './lib/dataBackup'
import {
  DEFAULT_TIMEZONE,
  dateInZone,
  deleteItineraryItem,
...
function MyScreen({ trips, remoteMode, user, onSignOut, onImportLocal, preferences, onPreferencesChange, onBackupImported }) {
  ...
  if (view === 'data') {
    return <DataBackupScreen remoteMode={remoteMode} onBack={() => setView('main')} onBackupImported={onBackupImported} />
  }
    ...
    <div className="my-subpage my-settings-screen">
      ...
      <section className="section-block settings-card-block">
        <div className="preference-toggle-row">
          <div><strong>페이지 전환 효과</strong><small>탭이나 화면을 바꿀 때 살짝 올라오며 나타나는 효과를 사용해요.</small></div>
...
function DataBackupScreen({ remoteMode, onBack, onBackupImported }) {
  ...
  const download = async () => {
    ...
    setStatus('')
    try {
      await downloadTripOnBackup()
      setStatus('JSON 백업을 만들었어요.')
    } catch (error) {
  ...
  const restore = async (event) => {
    ...
    setStatus('')
    try {
      await restoreTripOnBackup(file)
  ...
  return (
    <div className="my-subpage data-backup-screen">
      ...
      <section className="section-block settings-card-block">
        <div className="backup-section-head"><div><strong>JSON 백업</strong><small>현재 계정의 TRIP:ON 데이터베이스 내용을 파일로 보관해요.</small></div></div>
        <div className="backup-actions">
          <button className="outline-btn" disabled={!remoteMode || !!working} onClick={download}><Download size={17} /> {working === 'export' ? '내보내는 중…' : '내보내기'}</button>
          ...
          <input ref={inputRef} className="visually-hidden-file" type="file" accept="application/json,.json" onChange={restore} />
        </div>
        <p className="preference-note">같은 계정에서 만든 JSON을 합쳐서 복원해요. 이미지·PDF 원본 파일 자체는 JSON 안에 포함되지 않아요.</p>
  ...
  return (
    <div className="app-shell detail-shell">
      <main className="page detail-page">
        ...
        <div key={active} className={`detail-content detail-tab-${active}`}>
