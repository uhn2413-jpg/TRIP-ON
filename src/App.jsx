...
function linkDisplayMode(item) {
  return item?.link_display_mode || item?.linked_plan?.link_display_mode || 'plan'
}

function shouldShowLinkedActualCard(item) {
  if (!item?.linked_plan_id) return true
  const mode = linkDisplayMode(item)
  return mode === 'actual' || mode === 'both'
}
...
function ScheduleTab({ trip }) {
  ...
  const rootItems = dayItems.filter((item) => !item.group_parent_id && (!item.linked_plan_id || shouldShowLinkedActualCard(item)))
...
function ScheduleCard({ item, onClick }) {
  ...
  return (
    <div
      className={`schedule-card status-${item.status} ${actualOnly ? 'actual-only' : ''} ${item.is_visit_group ? 'visit-group' : ''}`}
      role="button"
      tabIndex={0}
      onClick={activate}
      onKeyDown={(event) => {
      ...
      <div className="schedule-card-copy">
        <div className="schedule-card-top">
          ...
          <span className="schedule-card-badges">
            {item.is_visit_group && <button type="button" className={`schedule-group-chip ${groupExpanded ? 'expanded' : ''}`} onClick={(event) => { event.stopPropagation(); setGroupExpanded((value) => !value) }}><Layers3 size={11} /> {groupCount}곳 {groupExpanded ? <ChevronUp size={11} /> : <ChevronDown size={11} />}</button>}
        ...
        {item.is_visit_group && !groupExpanded && summaryNames.length > 0 && <button type="button" className="schedule-linked-summary schedule-group-summary-toggle" onClick={(event) => { event.stopPropagation(); setGroupExpanded(true) }}>{summaryNames.slice(0, 3).join(' · ')}{summaryNames.length > 3 ? ` 외 ${summaryNames.length - 3}곳` : ''}<ChevronDown size={12} /></button>}
        ...
        {item.is_visit_group && groupExpanded && (
          <div className="schedule-group-expanded" onClick={(event) => event.stopPropagation()}>
            ...
            <button type="button" className="schedule-group-collapse" onClick={(event) => { event.stopPropagation(); setGroupExpanded(false) }}><ChevronUp size={13} /> 접기</button>
          </div>
        )}
  ...
  return (
    <SheetBackdrop onRequestClose={onClose}>
      <div className="create-sheet itinerary-detail-sheet" onPointerDown={(e) => e.stopPropagation()}>
        <div className="sheet-handle" />
        <div className="sheet-head"><button onClick={onClose}>닫기</button><span>{actualRecord ? '실제 방문 상세' : '일정 상세'}</span><button className="detail-edit-head" onClick={onEdit}><Pencil size={14} /> {actualRecord ? '실제 수정' : '수정'}</button></div>
...
function ItineraryLinkModal({ item, day, dayItems, allItems = dayItems, days = [day], onClose, onSaved }) {
  ...
  const [displayMode, setDisplayMode] = useState(item.link_display_mode || item.linked_plan?.link_display_mode || 'plan')
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
      ...
      <div className="create-sheet schedule-link-sheet" onPointerDown={(e) => e.stopPropagation()}>
        ...
        <div className="sheet-body">
          ...
          <div className="relation-display-mode">
            <small>표시 방식</small>
            <div className="relation-display-options">
              <label className={displayMode === 'plan' ? 'selected' : ''}><input type="radio" name="display-mode" checked={displayMode === 'plan'} onChange={() => setDisplayMode('plan')} /><span>예정 카드 중심</span></label>
              <label className={displayMode === 'actual' ? 'selected' : ''}><input type="radio" name="display-mode" checked={displayMode === 'actual'} onChange={() => setDisplayMode('actual')} /><span>실제 카드 중심</span></label>
              <label className={displayMode === 'both' ? 'selected' : ''}><input type="radio" name="display-mode" checked={displayMode === 'both'} onChange={() => setDisplayMode('both')} /><span>둘 다 보기</span></label>
            </div>
          </div>
