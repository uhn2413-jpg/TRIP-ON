...
function CoverPicker({ value, onChange }) {
  const [customValue, setCustomValue] = useState(value.toUpperCase())
  ...
  const applyCustomColor = () => {
    let next = customValue.trim()
    if (!next.startsWith('#')) next = `#${next}`
    if (!/^#[0-9A-Fa-f]{6}$/.test(next)) return
    onChange(next.toUpperCase())
  }

  const customValid = /^#?[0-9A-Fa-f]{6}$/.test(customValue.trim())
  ...
  return (
    <div className="cover-picker">
      <div className="palette-heading">
        <div>
          <strong>추천 색상</strong>
          <small>화면 전체와 자연스럽게 어울리는 밝은 톤이에요.</small>
        </div>
      ...
      <div className="custom-color-card">
        <span className="custom-color-swatch" style={{ backgroundColor: customValid ? (customValue.startsWith('#') ? customValue : `#${customValue}`) : value }} />
        <div className="custom-color-copy">
          <strong>직접 지정</strong>
          <small>HEX 색상값을 입력할 수 있어요.</small>
        </div>
        <div className="custom-color-input-wrap">
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter' && customValid) applyCustomColor()
            }}
            inputMode="text"
            maxLength={7}
            aria-label="직접 색상값 입력"
          />
          <button type="button" className="apply-color-btn" disabled={!customValid} onClick={applyCustomColor}>적용</button>
        </div>
      </div>
    </div>
  )
    ...
    <div className="modal-backdrop">
      <div className="create-sheet color-sheet">
        ...
        <div className="sheet-body">
          ...
          <h2>배경색을 바꿔볼까요?</h2>
          <p className="muted color-help">기본 색상에서 고르거나 아래 색상 선택기로 원하는 색을 직접 지정할 수 있어요.</p>
