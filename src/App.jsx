...
function ScheduleTab({ trip }) {
  ...
  const timelineTime = (item) => item.start_at || item.actual_start_at || null
  const hasTimelineTime = (item) => Boolean(timelineTime(item))
  const isBriefTimelineVisit = (item) => {
    if (item.import_source !== 'google_timeline' || item.start_at || !item.actual_start_at || !item.actual_end_at) return false
  ...
  const scheduledItems = dayItems
    .filter((item) => hasTimelineTime(item) && !isBriefTimelineVisit(item))
    .sort(byTimelineTime)
  const briefActualItems = dayItems
    .filter(isBriefTimelineVisit)
    .sort(byTimelineTime)
      ...
      {briefActualItems.length > 0 && (
        <section className="brief-actual-section">
          <button className="brief-actual-toggle" onClick={() => setShowBriefActual((value) => !value)}>
            <div>
              <span>짧은 방문 감지</span>
              <small>Timeline이 10초 미만으로 잡은 장소 {briefActualItems.length}개 · 일정표가 복잡해지지 않게 접어뒀어요.</small>
            </div>
            <span className="brief-actual-count">{briefActualItems.length}</span>
            {showBriefActual ? <ChevronUp size={17} /> : <ChevronDown size={17} />}
          </button>
          {showBriefActual && (
            <div className="brief-actual-list">
              {briefActualItems.map((item) => {
                const time = timeInZone(item.actual_start_at, item.actual_start_timezone || item.start_timezone || DEFAULT_TIMEZONE)
                return (
                  <button key={item.id} onClick={() => setViewingItem(item)}>
                    <time>{time}</time>
                    <span><strong>{item.title}</strong><small>{item.city || '실제 방문 감지'}</small></span>
...
function ScheduleCard({ item, onClick }) {
  ...
  const hasActualTime = Boolean(item.actual_start_at)
  const actualOnly = !hasPlannedTime && hasActualTime
  const startSource = actualOnly ? item.actual_start_at : item.start_at
  const endSource = actualOnly ? item.actual_end_at : item.end_at
  const startZone = actualOnly
    ? (item.actual_start_timezone || item.start_timezone || DEFAULT_TIMEZONE)
    : (item.start_timezone || DEFAULT_TIMEZONE)
  const endZone = actualOnly
    ? (item.actual_end_timezone || item.end_timezone || startZone)
  ...
  const actualStart = hasPlannedTime && hasActualTime
    ? timeInZone(item.actual_start_at, item.actual_start_timezone || item.start_timezone || DEFAULT_TIMEZONE)
    : ''
  const actualEnd = hasPlannedTime && item.actual_end_at
    ? timeInZone(item.actual_end_at, item.actual_end_timezone || item.end_timezone || item.start_timezone || DEFAULT_TIMEZONE)
    : ''
  const duration = formatTravelDuration(item.travel_minutes)
  const routeLabel = transportRouteLabel(item)
  return (
    <button className={`schedule-card status-${item.status} ${actualOnly ? 'actual-only' : ''}`} onClick={onClick}>
  ...
  return (
    <SheetBackdrop onRequestClose={requestClose}>
      <div className="create-sheet itinerary-sheet" onPointerDown={(e) => e.stopPropagation()}>
        <div className="sheet-handle" />
        <div className="sheet-head"><button onClick={requestClose}>취소</button><span>{item ? '일정 수정' : '일정 추가'}</span><div /></div>
        <div className="sheet-body itinerary-form">
          ...
          <button className="sheet-primary" disabled={!canSave || saving} onClick={save}>{saving ? '저장 중…' : item ? '수정 저장' : '일정 추가'}</button>
        </div>
      </div>
    </SheetBackdrop>
  )
